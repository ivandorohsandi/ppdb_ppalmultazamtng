...
<body>
    ...
    echo "
      <div class='px-4 py-5 my-5 text-center'>
        ...
        <div class='col-lg-6 mx-auto'>
          <p class='lead mb-4'>Mohon maaf untuk saat ini pendaftaran smp pondok pesantren al-multazam sudah ditutup</p>
        </div>
      </div>
      ";
  } else {
      ...
      <div class="container">
        <div class="row py-5">
          <h5 class="display-5 fw-bold lh-1 mb-4 text-capitalize fs-5">
            registrasi siswa baru <span class="badge bg-success">SMP</span>
          </h5>
          ...
          <div class="form-group row mt-2 mb-2">
            ...
            <div class="col-xxl-10 col-lg-12">
              <input type="text" class="form-control" placeholder="Masukkan Jenjang Pendidikan" value="SMP" name="jenjang" required readonly />
            </div>
          </div>
  ...
  if (isset($_POST['submit'])) {
    ...
    $CheckExist = "SELECT * FROM datappdb_smp_ppalmultazam WHERE nisn = '$nisn'";
    ...
    if ($resultCheckExist->num_rows > 0) {
      ...
      $queryJumlahPendaftar = "SELECT COUNT(*) as jumlah_pendaftar FROM datappdb_smp_ppalmultazam";
      ...
      if ($jumlahPendaftar >= $kuota) {
        ...
        if (in_array($fotoType, $allowedTypes) && $fotoSize <= $maxFileSize && in_array($sklType, $allowedTypes) && $sklSize <= $maxFileSize) {
          ...
          $query = "INSERT INTO datappdb_smp_ppalmultazam (tgl_daftar, nama_gelombang, nomor_pendaftaran,     nama_lengkap,    nik,    jenjang,    jenis_kelamin,    tempat_lahir,    tgl_lahir,    asal_sekolah,    nisn,    agama,     tinggi_badan,   berat_badan,   jml_sdr_kandung,    nama_ayah,    thn_lahir_ayah,    pekerjaan_ayah,    penghasilan_ayah,    pendidikan_ayah,   nama_ibu,     thn_lahir_ibu,    pekerjaan_ibu,    penghasilan_ibu,    pendidikan_ibu,  alamat_rumah,    rt,    rw,      kode_pos,    provinsi,    kabupaten_kota,    kecamatan,    kelurahan,    no_telp,    email,    foto,           skl,           status_pendaftaran)
                                                 VALUES ('$tgl_daftar', '$nama_gelombang', '$nomor_pendaftaran', '$nama_lengkap', '$nik', '$jenjang', '$jenis_kelamin', '$tempat_lahir', '$tgl_lahir', '$asal_sekolah', '$nisn', '$agama', '$tinggi_badan', $berat_badan, '$jml_sdr_kandung', '$nama_ayah', '$thn_lahir_ayah', '$pekerjaan_ayah', '$penghasilan_ayah', '$pendidikan_ayah', '$nama_ibu', '$thn_lahir_ibu', '$pekerjaan_ibu', '$penghasilan_ibu', '$pendidikan_ibu', '$alamat_rumah', '$rt', '$rw', '$kode_pos', '$provinsi', '$kabupaten_kota', '$kecamatan', '$kelurahan', '$no_telp', '$email', '$newfotoName', '$newSKLName', '$status_pendaftaran')";
          ...
          $status_url = "https://ivando.my.id/smp/form_penerimaan";
