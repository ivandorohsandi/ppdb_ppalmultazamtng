                            ...
                            if (!empty($fotoName)) {
                                ...
                                if (in_array($extension, $allowedExtensions)) { //hash name filename with sha-3 
                                    ...
                                    if (move_uploaded_file($fotoTmp, $newGambarPath)) {
                                        // Hapus gambar lama
                                        $queryDelete = "SELECT foto FROM datappdb_smp_ppalmultazam WHERE nomor_pendaftaran = '$nomor_pendaftaran'"; //delete image where id = $gambarId
                                        ...
                                        $queryUpdateSiswa = "UPDATE datappdb_smp_ppalmultazam SET  foto = '$newGambarName' WHERE nomor_pendaftaran = '$nomor_pendaftaran'";
                                        if ($koneksi->query($queryUpdateSiswa)) {
                            ...
                            if (!empty($sklName)) {
                                ...
                                if (in_array($extension, $allowedExtensions)) { //hash name filename with sha-3 
                                    ...
                                    if (move_uploaded_file($sklTmp, $newSKLPath)) {
                                        // Hapus gambar lama
                                        $queryDelete = "SELECT skl FROM datappdb_smp_ppalmultazam WHERE nomor_pendaftaran = '$nomor_pendaftaran'"; //delete image where id = $gambarId
                                        $resultDelete = $koneksi->query($queryDelete);
                                        if ($resultDelete->num_rows == 1) {
                                            $rowDelete = $resultDelete->fetch_assoc();
                                        ...
                                        $queryUpdateSiswa = "UPDATE datappdb_smp_ppalmultazam SET  skl = '$newSKLName' WHERE nomor_pendaftaran = '$nomor_pendaftaran'";
                                        if ($koneksi->query($queryUpdateSiswa)) {
                                ...
                                <div class="col-15">
                                    <div class="card"> 
                                        <div class="card-header">
                                            <h4 class="card-title">Edit Data Siswa SMP</h4>
                                        </div>
